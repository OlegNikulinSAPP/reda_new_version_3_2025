...
class ModelSelectorPanel(wx.Panel):
    def __init__(self, parent, db_path="baseReda.db"):
        ...
        self.SetBackgroundColour(wx.Colour(250, 252, 255))  # Светлый фон
        ...
        self.models_list.Bind(wx.EVT_LISTBOX_DCLICK, self.on_double_click)  # Двойной щелчок
    ...
    def on_search_text(self, event):  # noqa: unused-argument
    ...
    def on_search(self, event):  # noqa: unused-argument
        """Обработка поиска"""
    ...
    def on_model_select(self, event):  # noqa: unused-argument
    ...
    def on_double_click(self, event):
        """Обработка двойного щелчка по модели"""
        self.on_select(event)

    def on_select(self, event):  # noqa: unused-argument
        """Возвращает выбранную модель родительскому компоненту"""
        selection = self.models_list.GetSelection()
        if selection != wx.NOT_FOUND:
            ...
            try:
                ...
                if row:
                    # Создаем словарь параметров с ВСЕМИ полями
                    params = {}
...
class EDParametersPanel(wx.Panel):
    def __init__(self, parent, db_path="baseReda.db"):
        ...
        # Панель для параметров с прокруткой
        ...
        self.param_names = [
            ...
            ("I_nom", "Номинальный ток, А"),
            ("Turning", "Частота вращения, об/мин"),
            ("R_ColdWinding", "Сопротивление обмотки (хол.), Ом"),
            ("R_Insul", "Сопротивление изоляции, МОм"),
            ("U_accel", "Напряжение разгона, В"),
            ("BoringMoment", "Момент продерживания, кгс·м"),
            ("U_k_z", "Напряжение КЗ, В"),
            ("I_k_z", "Ток КЗ, А"),
            ("P_h_h", "Потери холостого хода, Вт"),
            ("I_Idling", "Ток холостого хода, А"),
            ("U_Idling", "Напряжение холостого хода, В"),
            ("P_k_z", "Потери КЗ, Вт"),
            ("Time_RunDown", "Время выбега, с"),
            ("VibrLevel", "Уровень вибрации, мм/с"),
            ("TurningMoment", "Крутящий момент, кгс·м"),
            ("P_HeatedWaste", "Потери нагрева, Вт"),
            ("U_MinInsulWinding", "Мин. напряжение изоляции обмотки, В"),
            ("U_InsulWinding", "Напряжение изоляции обмотки, В")
        ]

        # Создаем элементы управления для параметров
        for db_name, ru_name in self.param_names:
            if db_name == "ID":  # Пропускаем ID
                continue
            ...
            # Для некоторых параметров делаем поле ввода, для других - только чтение
            if db_name == "Model":
    ...
    def set_parameters(self, params):
        """Заполняет поля параметров из словаря params"""
    ...
    def on_save(self, event):  # noqa: unused-argument
        ...
        update_data = {}
        for db_name, ctrl in self.param_controls.items():
            if db_name != "Model":  # Модель не обновляем
                update_data[db_name] = ctrl.GetValue()

        # Формируем SQL-запрос
        set_clause = ", ".join([f"{k} = ?" for k in update_data.keys()])
        values = list(update_data.values())
        values.append(self.current_model_id)
